...
function toolboxReducer(state, action) {
  switch (action.type) {
    case "TOOLBOX_ACTION_CHANGE_STROKE": {
      const newState = { ...state};
      newState[action.payload.tool].stroke = action.payload.stroke;
      return newState;
    }

    case "TOOLBOX_ACTION_CHANGE_FILL" : {
      const newState = { ...state};
      newState[action.payload.tool].fill = action.payload.fill;
      return newState;
    }

    case "TOOLBOX_ACTION_CHANGE_SIZE" : {
      const newState = { ...state};
...
const ToolboxProvider = ({ children }) => {
  ...
  const changeStrokeHandler = (tool , stroke) => {
    dispatchToolboxAction({ 
      type: 'TOOLBOX_ACTION_CHANGE_STROKE',
      payload : {
        tool, 
        stroke,
      },
  ...
  const changeFillHandler = (tool , fill) => {
    dispatchToolboxAction({ 
      type: 'TOOLBOX_ACTION_CHANGE_FILL',
      payload : {
        tool, 
        fill,
      },
  ...
  const changeSizeHandler = (tool , size) => {
    dispatchToolboxAction({ 
      type: 'TOOLBOX_ACTION_CHANGE_SIZE',
      payload : {
        tool, 
        size,
      },
